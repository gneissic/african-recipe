...
const TanzaniaFood = () => {
    ...
    try {
      const fetchDataHandler = async () => {
        ...
        const response = await fetch(
          'https://african-recipe-c6fe5-default-rtdb.firebaseio.com/food/tanzania.json',
        );
        if (!response.ok) {
  ...
  return (
    <Fragment>
      <FoodHeader />
      <div className="mb-[3.5rem] flex flex-wrap justify-around gap-[2.5rem] px-[1.5rem] pt-[3rem]">
        {food.map((items) => (
          <FoodItems
            key={items.name}
            name={items.name}
